...
class Storage {
	 ...
	 *            The file directory containing the files. All the instance with
	 *            the same path have the same group.
	 */
	public Storage(final Gson gson, final Path storageLocation,
		...
		storePath = storageLocation.resolve(Paths.get(getClass()
				.getSimpleName()));
	}
	...
	private void prepareSingle(final Class<?> type) {
		retriverByType.put(type, threadPool.submit(new Callable<Object>() {
			...
			public Object call() throws Exception {
				return serializer.fromJson(fileHandling.load(objectPath(type)),
						type);
			}
	...
	/**
	 * Remove all data that was stored by instance of the same group. The group
	 * is implementation depended.
	 * 
	 * @throws IOException
	 */
