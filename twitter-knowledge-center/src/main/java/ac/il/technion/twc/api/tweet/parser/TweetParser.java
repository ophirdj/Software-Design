...
public class TweetParser {
	...
	private static class RunningFormat {
		...
		private final List<Pair<TweetFormat, List<Tweet>>> formats = new ArrayList<>();

		public RunningFormat(final List<TweetFormat> formats) {
			for (final TweetFormat parserFormat : formats)
				this.formats.add(new Pair<TweetFormat, List<Tweet>>(
		...
		public void parse(final String tweet) {
			for (final Iterator<Pair<TweetFormat, List<Tweet>>> iterator = formats
					.iterator(); iterator.hasNext();) {
				final Pair<TweetFormat, List<Tweet>> parserFormat = iterator
						.next();
				try {
