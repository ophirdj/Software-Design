...
class ServiceBuildingManager {
	...
	/**
	 * Inform the {@link ServiceBuildingManager} that the given type is
	 * supported by a {@link PropertyBuilder}
	 * 
	 * @param type
	...
	public void checkProperty(final Class<?> type) {
		try {
			final Constructor<?> ctor = type.getConstructor(List.class,
					List.class);
	...
	/**
	 * Inform the {@link ServiceBuildingManager} that the value of the given
	 * type is predefine to the given value
	 * 
	 * @param type
	 *            The type
	 * @param value
