...
public class AdvancedUsageTest {
		...
		public RetweetCounter(final TweetsRetriever retriever,
			...
			for (final Retweet retweet : retriever.getRetweets())
				try {
					...
					followersFromID
							.put(origin, followersFromID.get(origin) + 1);
	...
	/**
	 * A {@link Serializer} for {@link RetweetCounter}
	 * 
	 * @author Ophir De Jager
	 * 
	 */
	public static class RetweetCounterSerializer implements Serializer {

		private final Gson gson = new GsonBuilder().registerTypeAdapter(
				RetweetCounter.class, new TweetToLifeTimeJsonSerializer())
				.create();

		private static class TweetToLifeTimeJsonSerializer implements
				JsonSerializer<RetweetCounter>,
				JsonDeserializer<RetweetCounter> {
			@Override
			public RetweetCounter deserialize(final JsonElement json,
				...
				final Map<String, Integer> m = context.deserialize(
						jsonObject.get("followersFromID"),
			...
			public JsonElement serialize(final RetweetCounter counter,
				...
				for (final Entry<ID, Integer> e : counter.followersFromID
						.entrySet())
	...
	public final void usingPreDefinedPropertiesAndCustomSerializerForQuery()
		...
		final int numBase = 10;
		dataCenter.importData(parser.parse(TestUtils.generateTweets(numBase,
				10, 2, 0)));
		dataCenter.loadServices();
		final RetweetCounter counter = dataCenter
				.getService(RetweetCounter.class);

		for (int baseTweet = 0; baseTweet < numBase; ++baseTweet)
