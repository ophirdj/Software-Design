...
public class TemporalHistogramTest {
	...
	private final int[] valuesBase = new int[] { 1, 0, 4, 5, 0, 6, 7, 0, 0, 2,
			1, 3, 0, 0, 1, 10, 18, 0, 0 };
	private final int[] valuesRe = new int[] { 10, 0, 3, 2, 12, 0, 2, 0, 0, 2,
			1, 0, 7, 2, 0, 0, 0, 6, 1 };
...
//				0, 5 };
	...
	public void emptyHistogramShouldReturnAllZeros() {
		new TemporalHistogram(histogramFormatMock).get(
	...
	private Object[] testHistogram() {
		...
		return $(
				$(new int[] { 0, 0, 0, 0, 0, 0, 0 }, new int[] { 0, 0, 0, 0, 0,
						0, 0 }, new HashSet<>(), new HashSet<>(), new Date(),
						new Date()),
	...
	public void runHistogramTest(final int[] expectedBase,
			...
			final Set<Entry<Long, Integer>> dataRe, final Date from,
			final Date to) {
		when(dayMappingMock.getAllDaysBase()).thenReturn(dataBase);
		when(dayMappingMock.getAllDaysRe()).thenReturn(dataRe);
		new TemporalHistogram(dayMappingMock, histogramFormatMock)
				.get(from, to);
		verify(histogramFormatMock).formatHistogram(expectedBase, expectedRe);
	}
	...
	public void runIntervalsTest(final int from, final int to) {
		...
		when(dayMappingMock.getAllDaysBase()).thenReturn(
				makeSet(dates, valuesBase));
		when(dayMappingMock.getAllDaysRe())
				.thenReturn(makeSet(dates, valuesRe));
		new TemporalHistogram(dayMappingMock, histogramFormatMock).get(
				dates[from], dates[to]);
		verify(histogramFormatMock).formatHistogram(expectedBase, expectedRe);
	}

	private int[] createExpected(final int from, final int to,
			final int[] values) {
		final int[] $ = new int[7];
